...
class code_edit(QtGui.QWidget):
    ...
    def saveSnippet(self):
        text, ok = QtGui.QInputDialog.getText(self, 'Save Text Snippet', 'Provide a descriptive name for the snippet to save to your settings:')
        self.config.options('snippets')[text]=self.editor.toPlainText()
        self.list.addItem(text)
        self.config.config.set("snippets",text, self.config.config_encode(self.editor.toPlainText()))
        self.config.save_configuration()
        self.snippets = self.config.options('snippets')
        self.list.setCurrentIndex(self.list.findText(text))
        snip_list = list(self.snippets.keys())
        for snip in range(0,len(snip_list)):
            if snip_list[snip] == text:
                print (snip_list[snip])
                self.replace_snippet(snip)

    def select(self,product):
        self.c = self.Console({'model':self.model, 'viewer':self.viewer, 'selection':product})

    def __init__(self,viewer,config=None):
        ...
        width = self.savebutton.fontMetrics().boundingRect("Save").width() + 20
        self.savebutton.setMaximumWidth(width)
        self.tools.addWidget(self.savebutton)
        self.savebutton.setShortcut(QtGui.QKeySequence("CTRL+SHIFT+S"))
